        ...
        <div  class="contentBox">
            ...
            <div class="mastDetailHeader">
                ...
                <div class="rightBox rightBoxnone" >
                    <div>
						<div v-if="isShowFeng">
							<p><span>风切变曲线图</span></p>
							<div class="biao5">通 道 高 度 (m)</div>
							<e-charts ref="echarts5" class="echart" :style="{width:main2Style.width+'px',height:main2Style.height+'px'}" :options="chart5" :autoResize="true"
						...
						<div v-if="!isShowFeng">
							...
							<e-charts ref="echarts3" class="echart" :style="{width:(main3Style.width)+'px',height:main3Style.height+'px'}" :options="chart3" :autoResize="true"
							@legendselectchanged= "legendselect($event)"></e-charts>
						</div>
                    </div>
                    <div>
						<p><span>威布尔分布图</span> </p>
						<div class="biao4">频&nbsp; 率</div>
				...
				<div v-if="isShowFeng" class="rightrightBox rightBoxnone rightBoxnone3"  >
					<div :style="{width:(main3Style.width)+'px'}">
						<p><span>湍流曲线图</span></p>
						<div class="biao3">
							代表湍流 
							<div class="fanzhuan3">(lrep)</div>
					...
					<div :style="{width:main3Style.width+'px'}">
							...
							<div class="img_div" :style="{height:(main3Style.height)+'px'}">
								<img v-if="imageUrl2" :src="imageUrl2" :style="{height:(main3Style.height)+'px'}" class="avatar">
								<!-- <el-image
								v-if="imageUrl2"
								:style="{height:(main3Style.height)+'px'}" class="avatar"
								:src="imageUrl2"
								fit="fit">
				...
				<div v-if="!isShowFeng" class="rightrightBox rightBoxnone rightBoxnone3"  >
					<div :style="{width:main3Style.width+'px'}">
							...
							<div class="img_div" :style="{height:(main3Style.height)+'px'}">
								<img v-if="imageUrl1" :src="imageUrl1" :style="{height:(main3Style.height)+'px'}" class="avatar">
								<!-- <el-image
								v-if="imageUrl1"
								:style="{height:(main3Style.height)+'px'}" class="avatar"
								:src="imageUrl1"
    ...
    export default {
        ...
        data () {
            return {
				...
				loadingEchart: {
					...
					loading2: false,
					loading3: false,
				}, //axios.all不适合这用，改watch监听
				isTwoImage: false,   ////是否有湍流，是否显示两个图片，true=>2个图片
				isShowFeng: true   // 是否显示风切变 true 
            };
        },
		updated() {
			//屏幕变化触发
			...
			window.onresize = function temp() {
				that.main1Style.width = (document.documentElement.clientWidth*0.28-70);
				that.main1Style.height = (document.documentElement.clientHeight*0.33);
				that.main2Style.width = (document.documentElement.clientWidth*0.37-100);//
				that.main2Style.height = (document.documentElement.clientHeight*0.33);
				that.main3Style.width = (document.documentElement.clientWidth*0.315);
				that.main3Style.height = (document.documentElement.clientHeight*0.33);
				
				if(window.screen.width < 1900 && window.screen.width >= 1600){
					that.main1Style.width = (document.documentElement.clientWidth*0.28-60);
				}
				if(window.screen.width < 1900 && window.screen.width >= 1600 && window.screen.height == 900){
					that.main1Style.width = (document.documentElement.clientWidth*0.28-55);
				}
				if(window.screen.width < 1600 && window.screen.width >= 1400 ){
					that.main2Style.width = (document.documentElement.clientWidth*0.37-80);//
					
				}
				if(window.screen.width < 1400 && window.screen.width >= 1360){
					that.main1Style.width = (document.documentElement.clientWidth*0.28-45);
		...
		watch: {
			...
			loadingEchart: {
				...
				handler: function(newV,oldV){
					// console.log(newV.loading1,newV.loading2,newV.loading3);
					if(!newV.loading1 && !newV.loading2 && !newV.loading3){
        ...
        methods: {
            ...
            open (towerInfo) {
				...
				this.main1Style.width = (document.documentElement.clientWidth*0.28-70);
				this.main1Style.height = (document.documentElement.clientHeight*0.33);		
				this.main2Style.width = (document.documentElement.clientWidth*0.37-100);//
				this.main2Style.height = (document.documentElement.clientHeight*0.33);				
				this.main3Style.width = (document.documentElement.clientWidth*0.315);
				this.main3Style.height = (document.documentElement.clientHeight*0.33);
				
				
				if(window.screen.width < 1900 && window.screen.width >= 1600){
					this.main1Style.width = (document.documentElement.clientWidth*0.28-60);
				}
				if(window.screen.width < 1900 && window.screen.width >= 1600 && window.screen.height == 900){
					this.main1Style.width = (document.documentElement.clientWidth*0.28-55);
				}
				if(window.screen.width < 1600 && window.screen.width >= 1400 ){
					this.main2Style.width = (document.documentElement.clientWidth*0.37-80);//
					
				}
				if(window.screen.width < 1400 && window.screen.width >= 1360){
					this.main1Style.width = (document.documentElement.clientWidth*0.28-45);
            ...
            closeDetail () {
				...
				this.chart5 = this.getChart5DefaultValue();
				this.all_heightArr =[];
				this.new_height = "";
				this.heights = "";
			...
			selectChange(obj){
				...
				this.getMain3();
				this.getMain5();
				this.loadingEchart.loading1 = true;
				this.loadingEchart.loading2 = true;
				this.loadingEchart.loading3 = true;
			},
			getMain1(){
				// console.log(this.$store.state.app.projectInfo.project_id,this.towerInfo.name)
				this.chart1 = this.getChart1DefaultValue();
				...
				this.loadingEchart.loading1 = false;  
				if(res.status == 200){		
				...
				//    this.all_heightArr = res.data.data.all_height
				   this.new_height = res.data.data.height
			...
			getMain3(){
				...
				util.ajax.get('/wind/wei_turbulence', {
					...
					this.loadingEchart.loading2 = false;   
					if(res.status == 200){
					...
					//如果series为空数组，不显示湍流图，显示两个自定义图片
						...
						var weibull_nihe = res.data.data.weibull.nihe;
						
						
						//----------------
						let colors4 = ["#33C8C6","#00FF00","#F00188"]
						let colorsb4 = ["#B5C833","#00FF00","#F00188"]
						this.chart4 = this.getChart4DefaultValue(weibull_series,weibull_xAxis ,weibull_nihe);
						...
						for(var i=0 ;i<weibull_series.length; i++){
							...
							this.chart4.legend.data.push(
								{
									name:this.new_height+"m ", //柱状
								},
								{
									name:this.new_height+"m"
								})
							this.chart4.series.push(
								{
								name: this.new_height+"m ", //柱状,名字不能与线重复，别去空格
								type:'bar',
								// data:[0.1,0.14,0.1],
						...
						this.chart3.color.push(colorsmy[0]);
			...
			getMain5(){
				...
				util.ajax.get('/wind/wind_shear', {
					params:{
						project_id : data.project_id,
						mast_name : data.mast_name,
						
					}
				} ).then(res => {
					// console.log(res);
					if(res.data.code == 200){
						this.loadingEchart.loading3 = false;
						var lineArr = res.data.data.nihe_shear;
						var heightArr = res.data.data.height;
						var spdhhArr = res.data.data.spdhh;
						if(spdhhArr.length<=0){
							this.isShowFeng = false;
						}else{
							this.isShowFeng = true;
						}
						for(let i=0; i<=spdhhArr.length-1; i++ ){
							spdhhArr[i] = Number(String(spdhhArr[i]).replace(/^(.*\..{2}).*$/,"$1"))
						}
						var arrX = [];
						var arrAxis = [];
						var xAxisMax = Math.max.apply(null, spdhhArr);
						var xAxisMin = Math.min.apply(null, spdhhArr)

						var heightMax = Math.max.apply(null, heightArr);
						if(xAxisMin<0){
							xAxisMin = 0;
						}
						for(let i=0; i<(xAxisMax+1); i++){
							arrAxis.push(i);
						}
						...
						// console.log(arrAxis);
						// console.log(arrX);
						this.chart5 = this.getChart5DefaultValue(heightArr , spdhhArr ,arrX ,arrAxis,xAxisMax,xAxisMin ,heightMax, lineArr);
					}
...
// 					},
			...
			//风切变
			getChart5DefaultValue(heightArr , spdhhArr ,arrX,arrAxis,xAxisMax,xAxisMin, heightMax, lineArr){
				return{
					tooltip: {
						trigger: 'item',
						axisPointer: {
						...
						formatter: function(params){
							// console.log(params)
							return params.value[1] +"<br/>"+ params.seriesName +" : "+ params.value[0]
						}
					},
					grid: {
						left: '4%',
