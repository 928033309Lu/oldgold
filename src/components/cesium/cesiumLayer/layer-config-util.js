...
require('./cesium.ChineseTmsProviders')
...
export function loadLayerByConfig (layerData, redraw) {
  ...
  switch (String(layerConfig.type).toUpperCase()) {
    case 'INNER':
      layer = Cesium.ImageryLayer.getChinaLayer(layerConfig.url, layerConfig.options || {})
      break
    case 'TPL_IMG':
      options = {
        url: layerConfig.url,
      ...
      layer = new Cesium.ImageryLayer(new Cesium.UrlTemplateImageryProvider(options))
      break
    case 'WMS':
      // 添加WMS图层
      if (layerConfig.url && layerConfig.layers) {
        ...
        layer = new Cesium.ImageryLayer(new Cesium.WebMapServiceImageryProvider(options), getImageLayerOptions(layerData.styleConfig))
      }
      break
  }
